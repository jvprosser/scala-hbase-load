 ...
 */
object HBaseBulkLoadExampleFromFile {
  def main(args: Array[String]) {
    if (args.length < 3) {
      println("HBaseBulkLoadExampleFromFile {tableName} {columnFamily} {inputFile}")
      return
    }
    ...
    val sparkConf = new SparkConf().setAppName("HBaseBulkLoadExampleFromFile " +
